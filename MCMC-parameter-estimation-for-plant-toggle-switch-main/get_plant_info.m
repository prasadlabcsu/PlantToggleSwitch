...
%  Colorado State Univ
%  ------------------------------------------------------------------------
function Plant = get_plant_info(LINE_NAME, Plant_Idx, Day_Induce, Prefix)
...
%  ------------------------------------------------------------------------
%%
Num_Plants = length(Plant_Idx); 
Plant = {}; 
if strcmp(LINE_NAME, 'Toggle2-5-39') && Plant_Idx(5) == 21 
    % there is one day less in Plant Index 21. If this plant is drawn, we
    % replace it randomly with other plants in the same treatment. 
    Plant_Idx(5) = Idx(4, ceil(rand(1).*3)); 
end

for ii = 1:Num_Plants 
    Folder_Name = [LINE_NAME, '_', Prefix, '_Plant_Index_', sprintf('%d', Plant_Idx(ii))]; 
    ...
    if strcmp(LINE_NAME, 'Toggle2-5-49') || strcmp(LINE_NAME, 'TKK489-8-8')
    ...
    Log_mean = AA. data(:, 5); 
    Log_std  = AA. data(:, 6); 
    Plant(ii).idx  = info(Plant_Idx(ii)).idx; 
    Plant(ii).Trt1 = info(Plant_Idx(ii)).Trt1; 
    Plant(ii).Trt2 = info(Plant_Idx(ii)).Trt2; 
    Plant(ii).logmean = Log_mean; 
    Plant(ii).logstd  = Log_std; 
    Plant(ii).mean    = 2.^Log_mean; 
